...
function Courses({token ,userId}) {
 ...
 const [RateArr, setRateArr] = useState([0,0,0,0,0])
  ...
  const AddCourse = async () => {
    const respone = await axios.post("https://tuwaiq-project-hotel.herokuapp.com/hotel" , {
      name: name , 
  ...
  const deleteCourse = async (id) =>{
    const respone = await axios.delete("https://tuwaiq-project-hotel.herokuapp.com/hotel/" + id , 
    {headers: { authorization: `Bearer ${token}` }})
    setData(respone.data)
  }

  const deleteAll = async () => {
    const respone = await axios.delete("https://tuwaiq-project-hotel.herokuapp.com/deleteAll")
    setData(respone.data)
  }
